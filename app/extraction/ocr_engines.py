...
def _get_image_paths(file_path: str) -> List[str]:
    """
    ...
    # If PDF, convert to images (all pages)
    if path.suffix.lower() == '.pdf':
        try:
            ...
            logger.error(f"PDF conversion returned no images: {file_path}")
            return []
...
async def run_passport_eye(file_path: str) -> dict:
    """
    ...
    def _extract():
        try:
            ...
            image_paths = _get_image_paths(file_path)
            if not image_paths:
                return None, "Could not convert PDF to image or invalid file type"
            ...
            return None, "MRZ not detected on any page"
...
async def run_tesseract(file_path: str) -> dict:
    """
    ...
    def _extract():
        try:
            ...
            return None, "MRZ not found in any page or rotation"
...
async def run_easyocr(file_path: str) -> dict:
    """
